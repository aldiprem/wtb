...
def api_get_gifts():
    ...
    try:
        ...
        if not cur.fetchone():
            conn.close()
            return jsonify({'success': False, 'error': 'Tabel gift_scanned belum ada'}), 404
        ...
        for row in rows:
            ...
            gift_list.append({
                'id': row['rowid'], 'slug': slug, 'name': gift_name, 'number': gift_number,
                'message_id': row['message_id'], 'text': row['text'] or '',
                'lottie_url': lottie_url, 'fragment_url': fragment_url
            })
        
        conn.close()
        return jsonify({'success': True, 'data': gift_list, 'total': total, 'page': page, 'total_pages': total_pages})
...
def api_get_gift_detail(slug):
    ...
    try:
        conn = get_db_connection()
        if not conn: return jsonify({'success': False, 'error': 'Database tidak ditemukan'}), 500
        cur = conn.cursor()
        cur.execute("SELECT *, rowid FROM gift_scanned WHERE slug = ?", (slug,))
        row = cur.fetchone()
        conn.close()
        if not row: return jsonify({'success': False, 'error': 'Gift tidak ditemukan'}), 404
...
def api_get_stats():
    ...
    try:
        ...
        cur.execute("SELECT DISTINCT slug FROM gift_scanned")
        unique = len(cur.fetchall())
        conn.close()
        return jsonify({'success': True, 'stats': {'total': total, 'unique': unique}})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@gift_scanned_bp.route('/api/names')
def api_get_unique_names():
    """API: Mendapatkan semua unique names"""
    try:
        conn = get_db_connection()
        if not conn: return jsonify({'success': False, 'error': 'Database tidak ditemukan'}), 500
        cur = conn.cursor()
        ...
        rows = cur.fetchall()
        conn.close()
        names = set([row['slug'].rsplit('-', 1)[0] for row in rows])
        return jsonify({'success': True, 'names': sorted(list(names))})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
