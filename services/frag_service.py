...
def get_admin_stats():
    ...
    try:
        ...
        if WALLET_API_KEY and WALLET_MNEMONIC:
            try:
                wallet_balance = run_async(wallet_api.get_balance(WALLET_API_KEY, WALLET_MNEMONIC))
            except:
                pass
...
def add_bot():
    ...
    try:
        ...
        if success:
            # Start the bot
            await_start_bot(bot_token, bot_username)
            
            return jsonify({
                'success': True,
                'bot_username': bot_username,
                'bot_name': bot_name,
                'message': 'Bot added and started successfully'
            })
        else:
...
def start_bot(bot_id):
    ...
    try:
        ...
        success = await_start_bot(bot_detail['bot_token'], bot_detail['bot_username'])
...
def stop_bot(bot_id):
    ...
    try:
        ...
        success = await_stop_bot(bot_detail['bot_token'], bot_detail['bot_username'])
...
def delete_bot(bot_id):
    ...
    try:
        ...
        # Stop if running
        if bot_detail['status'] == 'running':
            await_stop_bot(bot_detail['bot_token'], bot_detail['bot_username'])
...
def await_start_bot(bot_token: str, bot_username: str) -> bool:
    """Start a bot (synchronous wrapper)"""
    try:
        import subprocess
        import sys
        ...
        env["IS_CLONE"] = "true"
        env["MASTER_BOT_TOKEN"] = os.getenv("BOT_TOKEN", "")
        
        script_path = Path(__file__).parent.parent / "fragment" / "fragment_bot.py"
        ...
        run_async(db_data.update_bot_status(bot_token, 'running', proc.pid))
        return True
    except Exception as e:
        logger.error(f"Error starting bot process: {e}")
        return False

def await_stop_bot(bot_token: str, bot_username: str) -> bool:
    """Stop a bot (synchronous wrapper)"""
    try:
        run_async(db_data.update_bot_status(bot_token, 'stopped'))
        return True
    except Exception as e:
        logger.error(f"Error stopping bot: {e}")
        return False