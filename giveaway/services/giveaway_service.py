...
# ==================== CEK KEHADIRAN DI CHAT ====================

@giveaway_bp.route('/check-membership', methods=['POST'])
def check_membership():
    ...
    try:
        ...
        if not user_id or not chat_id:
            return jsonify({'success': False, 'error': 'Missing user_id or chat_id'}), 400
        
        # Convert chat_id to integer (remove -100 prefix if present)
        chat_id_int = int(chat_id) if isinstance(chat_id, int) else int(chat_id)
...
@giveaway_bp.route('/membership-status/<int:user_id>/<giveaway_code>', methods=['GET'])
def get_membership_status(user_id, giveaway_code):
    """Get membership status for user in all required chats"""
    try:
        # Ambil data giveaway
        giveaway = db.get_on_giveaway(giveaway_code)
        if not giveaway:
            return jsonify({'success': False, 'error': 'Giveaway not found'}), 404
        
        giveaway_id = giveaway.get('giveaway_id', '')
        syarat = db.get_syarat(giveaway_id) if giveaway_id else 'None'
        
        # Cek apakah ada syarat Subscribe
        if 'Subscribe' not in syarat:
            return jsonify({'success': True, 'requirements': [], 'all_met': True})
        
        # Ambil daftar chat dari giveaway (disimpan di tabel giveaways)
        links_str = db.get_link(giveaway_id) if giveaway_id else ''
        
        # Parse chat IDs dari links (format: https://t.me/username atau t.me/username)
        chat_ids = []
        chat_usernames = []
        ...
        if links_str:
            ...
            for link in links:
                ...
                if 't.me/' in link:
                    ...
                    if username:
                        chat_usernames.append({
                            'link': link,
                            'username': username,
                            'checked': False
                        })
        
        # Untuk sementara, kita kembalikan daftar chat yang perlu dicek
        # Status akan diupdate oleh bot
        return jsonify({
            'success': True,
            'requirements': chat_usernames,
            'all_met': False
        })
    except Exception as e:
        print(f"Error in get_membership_status: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


# ==================== WEBHOOK UNTUK UPDATE STATUS MEMBERSHIP ====================
@giveaway_bp.route('/update-membership', methods=['POST'])
def update_membership_status():
    """Update membership status (called by bot)"""
    try:
        data = request.get_json()
        user_id = data.get('user_id')
        ...
        # Simpan status ke database atau cache
        # Untuk sementara, kita gunakan dictionary in-memory
        if not hasattr(db, 'membership_cache'):
            db.membership_cache = {}
        
        key = f"{user_id}_{giveaway_code}_{chat_username}"
        db.membership_cache[key] = {
            'is_member': is_member,
        ...
        print(f"Error in update_membership: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500