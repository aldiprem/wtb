...
# ==================== FUNGSI YANG DIBUTUHKAN OLEH frag_service.py ====================
...
async def get_panel_user(user_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error(f"Error getting panel user: {e}")
        return None


async def add_user_balance(user_id: int, amount: int) -> bool:
    """Add balance to user (alias for add_owner_balance)"""
    return await add_owner_balance(user_id, amount)


async def deduct_user_balance(user_id: int, amount: int) -> bool:
    """Deduct balance from user (alias for deduct_owner_balance)"""
    return await deduct_owner_balance(user_id, amount)


async def get_user_balance(user_id: int) -> int:
    """Get user balance (alias for get_owner_balance)"""
    return await get_owner_balance(user_id)


async def get_bot_logs(bot_token: str, limit: int = 50) -> List[Dict]:
    """Get logs for a specific bot from log files"""
    import glob
    from pathlib import Path
    ...
    # Look for log files
    log_pattern = f"logs/bot_{bot_username}*.log"
    log_files = glob.glob(log_pattern)
    
    if not log_files:
        # Try alternative pattern
        log_pattern = f"fragment/logs/bot_{bot_username}*.log"
        log_files = glob.glob(log_pattern)
...
def add_bot_log_sync(bot_token: str, level: str, message: str):
    ...
    try:
        ...
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT bot_username FROM cloned_bots WHERE bot_token = ?", (bot_token,))
        row = cursor.fetchone()
        conn.close()
        
        bot_username = row[0] if row else "unknown"
        
        log_file = log_dir / f"bot_{bot_username}.log"
...
async def get_bot_by_token(bot_token: str) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error(f"Error getting bot by token: {e}")
        return None
...
async def update_bot_status(bot_token: str, status: str, pid: int = None) -> bool:
    ...
    try:
        ...
        if pid:
            cursor.execute("""
                UPDATE cloned_bots SET status = ?, pid = ?, last_started = ? WHERE bot_token = ?
            """, (status, pid, now, bot_token))
        else:
            cursor.execute("""
                UPDATE cloned_bots SET status = ?, last_stopped = ? WHERE bot_token = ?
            """, (status, now, bot_token))
...
async def log_owner_activity(user_id: int, action: str, details: str = None):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO owner_activities (owner_id, action, details, timestamp)
            VALUES (?, ?, ?, ?)
        """, (user_id, action, details, get_jakarta_time_iso()))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"Error logging owner activity: {e}")
...
async def get_owner_activities(user_id: int, limit: int = 50) -> List[Dict]:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT action, details, timestamp FROM owner_activities
            WHERE owner_id = ? ORDER BY timestamp DESC LIMIT ?
        """, (user_id, limit))
        rows = cursor.fetchall()
        conn.close()
...
async def create_deposit(user_id: int, amount: int, payment_method: str = None, 
                         payment_proof: str = None) -> Optional[int]:
    """Create a new deposit request"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        order_id = f"DEP_{user_id}_{int(datetime.now().timestamp())}"
        now = get_jakarta_time_iso()
        
        cursor.execute("""
            INSERT INTO deposits (owner_id, order_id, amount, payment_method, payment_proof, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)
        """, (user_id, order_id, amount, payment_method, payment_proof, now, now))
...
async def update_deposit_status(deposit_id: int, status: str) -> bool:
    ...
    try:
        ...
        cursor.execute("""
            UPDATE deposits SET status = ?, updated_at = ?, completed_at = ? WHERE id = ?
        """, (status, now, now if status == 'completed' else None, deposit_id))
        
        conn.commit()
        
        # If completed, add balance
        if status == 'completed':
            cursor.execute("SELECT owner_id, amount FROM deposits WHERE id = ?", (deposit_id,))
            row = cursor.fetchone()
            if row:
                await add_owner_balance(row[0], row[1], f"deposit_{deposit_id}", "Deposit completed")
...
async def save_bot_fragment_config(bot_token: str, config_data: dict) -> bool:
    """Save bot fragment configuration"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        now = get_jakarta_time_iso()
        ...
        cursor.execute("""
            INSERT OR REPLACE INTO bot_config_templates (bot_token, config_type, config_data, updated_at)
            VALUES (?, 'fragment', ?, ?)
        """, (bot_token, json.dumps(config_data), now))
...
async def get_bot_fragment_config(bot_token: str) -> Optional[dict]:
    """Get bot fragment configuration"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("""
            SELECT config_data FROM bot_config_templates 
            WHERE bot_token = ? AND config_type = 'fragment'
        """, (bot_token,))
        row = cursor.fetchone()
        conn.close()
...
async def save_bot_wallet_config(bot_token: str, config_data: dict) -> bool:
    """Save bot wallet configuration"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        now = get_jakarta_time_iso()
...
async def get_bot_wallet_config(bot_token: str) -> Optional[dict]:
    """Get bot wallet configuration"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("""
            SELECT config_data FROM bot_config_templates 
            WHERE bot_token = ? AND config_type = 'wallet'
        """, (bot_token,))
        row = cursor.fetchone()
        conn.close()
...
async def create_panel_session(user_id: int, session_token: str = None, 
                               ip_address: str = None, user_agent: str = None) -> str:
    """Create panel session"""
    return await create_owner_session(user_id, ip_address, user_agent)


async def validate_panel_session(session_token: str) -> Optional[Dict]:
    """Validate panel session"""
    return await validate_owner_session(session_token)


async def delete_panel_session(session_token: str) -> bool:
    """Delete panel session"""
    return await delete_owner_session(session_token)


async def authenticate_panel_user(username: str, password: str) -> Optional[Dict]:
    """Authenticate panel user"""
    return await authenticate_bot_owner(username, password)


async def create_panel_user(username: str, password: str, owner_name: str = None,
                           email: str = None, expires_days: int = 30) -> Optional[int]:
    """Create panel user"""
    return await create_bot_owner(username, password, owner_name, email, None, expires_days)