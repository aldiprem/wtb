...
(function() {
    ...
    const closePlaceBetModal = document.getElementById('closePlaceBetModal');
    const confirmBetBtn = document.getElementById('confirmBetBtn');
    const modalBetAmount = document.getElementById('modalBetAmount');
    const modalUserBalanceSpan = document.getElementById('modalUserBalance');
    const pctBtns = document.querySelectorAll('.pct-btn');
    const tabBtns = document.querySelectorAll('.tab-btn');
    const tabContents = document.querySelectorAll('.tab-content');
    const riskTriggerBtn = document.getElementById('riskTriggerBtn');
    const riskPanel = document.getElementById('riskPanel');
    const closeRiskPanel = document.getElementById('closeRiskPanel');
    ...
    async function updateModalBalance() {
        try {
            ...
            const user = tg.initDataUnsafe?.user;
            if (user && user.id) {
                const response = await fetch('/api/games/balance/' + user.id);
                const data = await response.json();
                if (data.success) {
                    currentBalance = data.balance;
                    modalUserBalanceSpan.textContent = currentBalance.toFixed(2) + ' TON';
                }
            }
        } catch (error) {
            console.error('Error loading modal balance:', error);
        }
    }
    ...
    pctBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            let betValue = 0;
            if (pct === 100) {
                betValue = currentBalance;
            } else {
                betValue = (currentBalance * pct) / 100;
            }
            modalBetAmount.value = betValue.toFixed(2);
        });
    });
    ...
    placeBetBtn.addEventListener('click', async () => {
        ...
        placeBetModal.style.display = 'flex';
    });
    ...
    placeBetModal.addEventListener('click', (e) => {
        if (e.target === placeBetModal) {
            placeBetModal.style.display = 'none';
        }
    });

    // Confirm bet - update main bet amount
    confirmBetBtn.addEventListener('click', () => {
        const betValue = parseFloat(modalBetAmount.value);
        ...
        placeBetModal.style.display = 'none';
        alert(`Taruhan ${betValue} TON telah diset!`);
    });

    // Risk panel toggle
    riskTriggerBtn.addEventListener('click', () => {
        if (riskPanel.style.display === 'none') {
            riskPanel.style.display = 'block';
            riskTriggerBtn.querySelector('.fa-chevron-up').style.transform = 'rotate(180deg)';
        } else {
            riskPanel.style.display = 'none';
            riskTriggerBtn.querySelector('.fa-chevron-up').style.transform = 'rotate(0deg)';
        }
    });

    closeRiskPanel.addEventListener('click', () => {
        riskPanel.style.display = 'none';
        riskTriggerBtn.querySelector('.fa-chevron-up').style.transform = 'rotate(0deg)';
    });
    ...
    riskOptions.forEach(opt => {
        opt.addEventListener('click', () => {
            ...
            currentRisk = opt.dataset.risk;
            
            // Update the old risk buttons (for compatibility)
            document.querySelectorAll('.risk-btn').forEach(btn => {
                if (btn.dataset.risk === currentRisk) {
                    btn.classList.add('active');
                } else {
    ...
    // Update existing playGame to use the bet amount from input
    async function playGame() {
    ...
    // Also update the existing playBtn event listener
    const playBtn = document.getElementById('playBtn');
    if (playBtn) {
        // Remove old listener and add new one
        const newPlayBtn = playBtn.cloneNode(true);
        playBtn.parentNode.replaceChild(newPlayBtn, playBtn);
        newPlayBtn.addEventListener('click', playGame);
    }
