...
"""
        ...
        buttons = [[Button.inline("✅ KONFIRMASI", data=f"verify:{verification_id}")]]
...
async def verify_callback(event):
    """Handle verifikasi dari channel atau user - SATU HANDLER UNTUK SEMUA"""
    verification_id = event.data_match.group(1)
    clicker_id = event.sender_id
    
    # Ambil data pending dari DATABASE (bukan memory)
    ...
    # KONFIRMASI BERHASIL
    await event.answer("✅ Verifikasi berhasil! Menambahkan ke marketplace...", alert=True)
    
    # Simpan username ke database usernames
...
Username **@{pending['username']}** telah diverifikasi dan berhasil ditambahkan ke marketplace.

📝 **Detail:**
• Penjual: {pending['seller_name']} (ID: `{pending['seller_id']}`)
• Harga: Rp {format_price(pending['price'])}
• Deskripsi: {pending['description'] if pending['description'] else '-'}
...
Username **@{pending['username']}** telah dikonfirmasi oleh pemiliknya dan berhasil ditambahkan ke marketplace.
...
"""
        ...
        await event.edit(f"❌ Gagal menambahkan username @{pending['username']}. Mungkin sudah terdaftar.")
        db.delete_pending_verification(verification_id)
...
"""
        ...
        if verification_id:
            db.update_pending_price_description(verification_id, user_states[user_id]['price'], description)
...
async def admin_list_pending(event):
    ...
    for p in pendings:
        ...
        msg += f"💰 Harga: {format_price(p['price']) if p['price'] else 'Belum diisi'}\n"
...
async def main():
    ...
    db.init_database()
    db.init_pending_table()  # Buat tabel pending_verifications jika belum ada
