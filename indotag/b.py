...
async def send_verification_to_channel(channel_id, username_input, seller_id, seller_name, price, description):
    """Kirim verifikasi ke channel - SIMPLE, tanpa ID rumit"""
    try:
        # Cek akses bot
        try:
...
"""
        ...
        buttons = [[Button.inline("✅ KONFIRMASI", data=f"confirm_channel:{username_input}:{seller_id}:{price}:{description}")]]
...
async def send_verification_to_user(user_id, username_input, seller_id, seller_name, price, description):
    """Kirim verifikasi ke user - SIMPLE"""
    try:
...
"""
        ...
        buttons = [[Button.inline("✅ KONFIRMASI", data=f"confirm_user:{username_input}:{seller_id}:{price}:{description}")]]
...
@bot.on(events.CallbackQuery(pattern=r"confirm_channel:([^:]+):([^:]+):([^:]+):(.*)"))
async def confirm_channel_callback(event):
    """Handle konfirmasi dari channel"""
    username_input = event.data_match.group(1)
    seller_id = int(event.data_match.group(2))
    price = int(event.data_match.group(3))
    description = event.data_match.group(4)
    
    channel_id = event.chat_id
    
    # Simpan ke database
    success = db.add_username(username_input, seller_id, price, description)
    
    if success:
        await event.edit(f"""
✅ **VERIFIKASI BERHASIL!**

Username **@{username_input}** telah diverifikasi dan masuk ke marketplace.
...
Username **@{username_input}** telah dikonfirmasi oleh pemiliknya.
...
"""
        ...
        try:
            await bot.send_message(
                OWNER_ID,
                f"📢 Username @{username_input} terjual oleh {seller_id} dengan harga Rp {format_price(price)}"
            )
        except:
            pass
    else:
        await event.edit(f"❌ Gagal: Username @{username_input} sudah terdaftar!")

@bot.on(events.CallbackQuery(pattern=r"confirm_user:([^:]+):([^:]+):([^:]+):(.*)"))
async def confirm_user_callback(event):
    """Handle konfirmasi dari user"""
    username_input = event.data_match.group(1)
    seller_id = int(event.data_match.group(2))
    price = int(event.data_match.group(3))
    description = event.data_match.group(4)
...
"""
        ...
        try:
            await bot.send_message(
                OWNER_ID,
                f"📢 Username @{username_input} dijual oleh {seller_id} dengan harga Rp {format_price(price)}"
            )
        except:
            pass
    else:
...
""")
        ...
        if target_type in ['channel', 'supergroup', 'group']:
            success, message = await send_verification_to_channel(
                target_id, username, user_id, seller_name, price, description
            )
        else:
