...
def init_db():
    ...
    cursor = conn.cursor()# tmp.py - Database handler untuk tampilan website (VERSI MYSQL)
import json
from db_config import get_db_connection, IMAGE_BASE_URL

# ==================== KONFIGURASI ====================

def get_website_endpoint_from_db(website_id):
    """Ambil endpoint website dari database"""
    try:
        conn = get_db_connection()
...
def convert_hash_to_url(image_hash, endpoint):
    ...
    if not image_hash or len(image_hash) != 35 or not endpoint:
        return image_hash
    return f"{IMAGE_BASE_URL}{endpoint}={image_hash}"

def convert_banner_hash_to_url(banner_item, endpoint):
    """
    Konversi banner item (bisa string hash atau dict dengan hash) ke URL
    """
    if not endpoint:
        return banner_item
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS images (
        id INT AUTO_INCREMENT PRIMARY KEY,
        hash VARCHAR(35) UNIQUE NOT NULL,
    ...
    conn.commit()
    conn.close()
    print("✅ MySQL Database initialized with new structure (tables: tampilan, promo, website_templates, images)")
...
# ==================== FUNGSI UNTUK GAMBAR (IMAGE HANDLER) ====================

def save_image_metadata(hash_value, filename, size):
...
def get_image_metadata(hash_value):
    """Ambil metadata gambar berdasarkan hash"""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute('SELECT * FROM images WHERE hash = %s', (hash_value,))
        return cursor.fetchone()
    except Exception as e:
        print(f"⚠️ Failed to get image metadata: {e}")
        return None
    finally:
        if conn:
            conn.close()

def delete_image_metadata(hash_value):
    """Hapus metadata gambar dari database"""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('DELETE FROM images WHERE hash = %s', (hash_value,))
        conn.commit()
...
def get_tampilan(website_id):
    ...
    if row:
        ...
        endpoint = get_website_endpoint_from_db(website_id)
            ...
            try:
                ...
                if isinstance(banners_data, list):
                    ...
                    for banner in banners_data:
                        converted = convert_banner_hash_to_url(banner, endpoint)
                        converted_banners.append(converted)
                ...
                if isinstance(promos_data, list):
                    for promo in promos_data:
                        if isinstance(promo, dict):
                            banner_val = promo.get('banner', '')
                            if banner_val and len(banner_val) == 35 and endpoint:
                                promo['banner'] = convert_hash_to_url(banner_val, endpoint)
...
def save_promos(website_id, promos_data):
    ...
    promos_data: array of promo objects dengan banner berupa HASH 35 karakter
    """
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    
    # Clean promos data - pastikan banner adalah hash (bukan URL)
    clean_promos = []
    for promo in promos_data:
        clean_promo = {
            'id': promo.get('id'),
            ...
            'never_end': promo.get('never_end', False),
            'notes': promo.get('notes', ''),
            'active': promo.get('active', True)
        }
        
        # Banner bisa berupa hash 35 karakter
        banner_value = promo.get('banner', '')
        if banner_value and len(banner_value) == 35:
            clean_promo['banner'] = banner_value
        elif banner_value and 'imgg.companel.shop' in banner_value:
            # Extract hash dari URL
            import re
            match = re.search(r'=([a-f0-9]{35})', banner_value)
            if match:
                clean_promo['banner'] = match.group(1)
            else:
                clean_promo['banner'] = ''
        else:
            clean_promo['banner'] = ''
        
        clean_promos.append(clean_promo)
    
    promos_json = json.dumps(clean_promos)
