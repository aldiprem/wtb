...
(function() {
    ...
    class FontManager {
        ...
        // ✨ DIPERBAIKI: Memanggil API untuk load Google Fonts
        async loadGoogleFonts(query = '') {
            try {
                ...
                // Fallback: gunakan data dari font-animations.js
                const fallbackFonts = [
                    { family: 'Inter', category: 'sans-serif' },
                    { family: 'Roboto', category: 'sans-serif' },
                    { family: 'Open Sans', category: 'sans-serif' },
                ...
                const filtered = query 
                    ? fallbackFonts.filter(f => f.family.toLowerCase().includes(query.toLowerCase()))
                    : fallbackFonts;
                    
                this.renderGoogleFonts(filtered);
            }
        }
        ...
        renderKeyframeProperties() {
            ...
            container.innerHTML = `
                <div class="keyframe-editor">
                    ...
                    <div class="form-group">
                        <label>Opacity</label>
                        <input type="range" min="0" max="1" step="0.1" value="${keyframe.properties.opacity || 1}">
                    </div>
                    <div class="form-group">
                        <label>Scale X</label>
                        <input type="range" min="0" max="3" step="0.1" value="${keyframe.properties.transform?.scaleX || 1}">
                    </div>
                    <div class="form-group">
                        <label>Scale Y</label>
                        <input type="range" min="0" max="3" step="0.1" value="${keyframe.properties.transform?.scaleY || 1}">
                    </div>
                </div>
            `;
        }
