...
(function() {
    ...
    let loadingStates = {
        ...
        vouchers: 'loading',
        activities: 'loading',
        balance: 'loading' // Tambahkan balance
    };
    ...
    function showLoading(show = true) {
        // Hanya untuk loading awal, jangan tampilkan loading overlay untuk background loading
        if (loadingStates.user === 'loading' || loadingStates.website === 'loading') {
            if (elements.loadingOverlay) {
                elements.loadingOverlay.style.display = show ? 'flex' : 'none';
            }
        } else {
            // Setelah user dan website loaded, loading overlay sudah tidak perlu
            if (elements.loadingOverlay) {
    ...
    function updateLoadingState(dataType, status) {
        ...
        // Hanya render ulang jika halaman yang sedang aktif
        const activePage = document.querySelector('.nav-item.active')?.dataset.page;
        if (activePage) {
            switch (activePage) {
                case 'home': 
                    if (dataType === 'products' || dataType === 'tampilan') {
                        renderHomePage(); 
                    }
                    break;
                case 'aktivitas': 
                    ...
                    break;
                case 'promo': 
                    if (dataType === 'promos' || dataType === 'vouchers') {
                        renderPromoPage(); 
                    }
                    break;
                case 'bank': 
                    if (dataType === 'transactions' || dataType === 'rekening' || dataType === 'balance') {
                        renderBankPage(); 
                    }
                    break;
                case 'profile': 
                    if (dataType === 'user' || dataType === 'transactions' || dataType === 'vouchers') {
                        renderProfilePage(); 
                    }
                    break;
            }
    ...
    function startAutoRefresh() {
        // Refresh transaksi dan balance setiap 30 detik
        ...
        autoRefreshIntervals.transactions = setInterval(() => {
            if (currentWebsite && currentUser) {
                loadUserBalance(true); // silent refresh
            }
        }, 30000); // 30 detik
        
        // Refresh status deposit pending setiap 10 detik
    ...
    async function checkAllPendingTransactions() {
        ...
        if (statusChanged) {
            ...
            // Update tampilan jika sedang di halaman bank
            const activePage = document.querySelector('.nav-item.active')?.dataset.page;
            if (activePage === 'bank') {
                renderBankPage();
            }
    ...
    function renderHomePage() {
        ...
        const displayLayanan = isDataLoaded('products') ? layananList : [];
        const displayProducts = isDataLoaded('products') ? filteredItems : [];
    ...
    function renderLayananGrid(layananData) {
        ...
        return layananData.map(layanan => `
            <div class="layanan-card" 
                 onclick="window.website.filterByLayanan('${escapeHtml(layanan.nama)}')">
    ...
    function renderProductsGrid(products) {
        ...
        return pageItems.map(item => {
            ...
            return `
                <div class="product-card ${!item.item_ready ? 'sold' : ''}" 
                     onclick="window.website.viewProduct(${item.id})">
    ...
    function renderLayananFilter() {
        ...
        if (uniqueLayanan.length === 0) {
            return '<div class="filter-bubbles"><span class="filter-bubble">Tidak ada layanan</span></div>';
        }
    ...
    function renderAplikasiFilter() {
        ...
        if (uniqueAplikasi.length === 0) {
            return '<div class="filter-bubbles"><span class="filter-bubble">Tidak ada aplikasi</span></div>';
        }
    ...
    function renderAktivitasPage() {
        ...
        const displayActivities = isDataLoaded('activities') ? aktivitasList : [];
    ...
    function renderPromoPage() {
        const displayPromos = isDataLoaded('promos') ? promosList : [];
    ...
    function renderProfilePage() {
        ...
        const displayBalance = isDataLoaded('balance') ? formatRupiah(balance) : 'Rp 0';
        const displayVoucherCount = isDataLoaded('vouchers') ? userVouchers.length : '0';
        const displayPurchaseCount = isDataLoaded('transactions') ? transactions.filter(t => t.type === 'pembelian').length : '0';
    ...
    async function loadUserBalance(silent = false) {
        ...
        try {
            ...
            // Gunakan endpoint yang benar - langsung dari users_service
            const response = await fetchWithRetry(
            ...
            if (response.success) {
                ...
                // Jika ada transaksi baru, update juga
                if (response.transactions) {
                    transactions = response.transactions || [];
                }
            } else {
                // Fallback ke endpoint transaksi
        ...
        try {
            ...
            if (response.success && response.deposit) {
                ...
                if (status === 'success') {
                    ...
                    const transaction = transactions.find(t => t.id == currentDeposit.id);
                    if (transaction) {
                        transaction.status = 'expired';
                        // Render ulang halaman bank jika sedang aktif
                        const activePage = document.querySelector('.nav-item.active')?.dataset.page;
                        if (activePage === 'bank') {
                            renderBankPage();
                        }
    ...
    async function init() {
        ...
        try {
            ...
            // Load website dan user terlebih dahulu (wajib)
