...
(function() {
    ...
    async function loadAllData() {
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                // Update masing-masing data dan loading state
                if (d.products) {
            ...
            updateLoadingState('vouchers', 'error');
            updateLoadingState('activities', 'error');
        }
    }
    ...
    // ==================== RENDER FUNCTIONS WITH SKELETON ====================
    ...
    function applyDynamicFonts() {
        if (!isDataLoaded('tampilan')) return;
    ...
    function renderFilterBubbles() {
        ...
        if (layananFilter && isDataLoaded('products')) {
            let html = '';
        ...
        if (aplikasiFilter && currentFilters.layanan && isDataLoaded('products')) {
            const layananData = productsData.find(l => l.layanan_nama === currentFilters.layanan);
            const aplikasiList = layananData?.aplikasi || [];
    ...
    function openDepositModal() {
        ...
        if (select) {
            ...
            if (isDataLoaded('rekening') && allRekeningList.length > 0) {
                ...
                options += '<option disabled>Memuat data rekening...</option>';
            }
